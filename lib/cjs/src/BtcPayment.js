...
const bitcoinselect_1 = __importDefault(require("bitcoinselect"));
...
class BtcPayment {
    ...
    static getBtcSigner = async (mnemonic, btcNetwork = BtcNetwork_js_1.default.Mainnet, accountIndex = 0, changeIndex = 0, addressIndex = 0, addressReuse = true) => {
        const btcAccount = await BtcWallet_js_1.default.getAccountFromMnemonic(mnemonic, accountIndex);
        const btcAddressSigner = await BtcWallet_js_1.default.getAddressFromAccount(btcAccount, changeIndex, addressIndex);
        ...
        const addressNext = addressReuse
            ? payment.address
            : await BtcWallet_js_1.default.generateBtcAddress(await BtcWallet_js_1.default.getAddressFromAccount(btcAccount, changeIndex + 1, addressIndex), btcNetwork);
        return {
            payment,
    ...
    static _utxoOptimizer = async (signer, target, signerUTXOList, txFee, additionalFee = 0) => {
        ...
        const feeRate = txFee;
        const selectedUTXO = (0, bitcoinselect_1.default)(signerUTXOList, target, feeRate);
