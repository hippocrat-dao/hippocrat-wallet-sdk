...
const BtcRpcNode_1 = __importDefault(require("./BtcRpcNode.js"));
const BtcNetwork_1 = __importDefault(require("./enums/BtcNetwork.js"));
const BtcRpcUrl_1 = __importDefault(require("./enums/BtcRpcUrl.js"));
const TxFee_1 = __importDefault(require("./enums/TxFee.js"));
const BtcWallet_1 = __importDefault(require("./BtcWallet.js"));
class BtcPayment {
    // Account to pay transaction
    static getBtcSigner = async (mnemonic, btcNetwork = BtcNetwork_1.default.Mainnet, accountIndex = 0, changeIndex = 0, addressIndex = 0, addressReuse = true) => {
        const btcAccount = await BtcWallet_1.default.getAccountFromMnemonic(mnemonic, accountIndex);
        const btcAddressSigner = await BtcWallet_1.default.getAddressFromAccount(btcAccount, changeIndex, addressIndex);
        ...
        const addressNext = addressReuse ? payment.address
            : await BtcWallet_1.default.generateBtcAddress(await BtcWallet_1.default.getAddressFromAccount(btcAccount, changeIndex + 1, addressIndex), btcNetwork);
        return {
            payment,
    ...
    static writeOnBtc = async (signer, messageList, txFee = TxFee_1.default.Average) => {
        // signerUTXO to spend
        const btcRpcUrl = await this._getSignerNetwork(signer);
        const signerUTXOList = await BtcRpcNode_1.default.getUTXOList(signer.payment.address, btcRpcUrl);
    ...
    static transferBtc = async (signer, receiverList, txFee = TxFee_1.default.Average) => {
        // signerUTXO to spend
        const btcRpcUrl = await this._getSignerNetwork(signer);
        const signerUTXOList = await BtcRpcNode_1.default.getUTXOList(signer.payment.address, btcRpcUrl);
        // get optimized transaction  
    ...
    static _signAndBroadcastTx = async (signer, psbt) => {
        ...
        const btcRpcUrl = await this._getSignerNetwork(signer);
        return await BtcRpcNode_1.default.broadcastTx(tx.toHex(), btcRpcUrl);
    };
    ...
    static _getSignerNetwork = async (signer) => {
        return signer.payment.network === bitcoin.networks.bitcoin ?
            BtcRpcUrl_1.default.Mainnet : BtcRpcUrl_1.default.Testnet;
    };
}
