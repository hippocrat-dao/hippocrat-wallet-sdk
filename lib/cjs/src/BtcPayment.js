...
class BtcPayment {
    ...
    static getBtcSigner = async (mnemonic, btcNetwork = BtcNetwork_1.default.Mainnet, accountIndex = 0, addressIndex = 0, addressReuse = true) => {
        const btcAccount = await BtcWallet_1.default.getAccountFromMnemonic(mnemonic, accountIndex);
        const btcAddressSigner = await BtcWallet_1.default.getAddressFromAccount(btcAccount, 0, addressIndex);
        ...
        const addressNext = addressReuse ? payment.address
            : await BtcWallet_1.default.generateBtcAddress(await BtcWallet_1.default.getAddressFromAccount(btcAccount, addressIndex + 1), btcNetwork);
        return {
            payment,
