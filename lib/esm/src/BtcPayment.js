...
class BtcPayment {
    ...
    static getBtcSigner = async (mnemonic, btcNetwork = BtcNetwork.Mainnet, accountIndex = 0, addressIndex = 0, addressReuse = true) => {
        const btcAccount = await BtcWallet.getAccountFromMnemonic(mnemonic, accountIndex);
        const btcAddressSigner = await BtcWallet.getAddressFromAccount(btcAccount, 0, addressIndex);
