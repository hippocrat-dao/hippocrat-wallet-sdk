...
describe('LNPayment create invoice test', () => {
    it('invoice pub key should be pub key of signer', async() => {
        ...
        const btcAccountPotential : hippocrat.BtcAccount = await hippocrat.BtcWallet.getAccountFromMnemonic(mnemonic);
        const btcAddressPotential : hippocrat.BtcAccount = await hippocrat.BtcWallet.getAddressFromAccount(btcAccountPotential);
        const lightningIndex : number = 0; // 0 is default, let's use address child for LNPayment
        const LNSigner : hippocrat.BtcAccount = await hippocrat.BtcWallet.getChildFromAddress(btcAddressPotential, lightningIndex);
        const btcAddressString : string = await hippocrat.BtcWallet.generateBtcAddress(LNSigner);
        // When
